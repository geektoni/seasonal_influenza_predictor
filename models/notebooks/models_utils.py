...
def generate_labels_sum():
    # Get all features files and sort the list
    file_list = os.listdir("./../data/austria/old_data")
    file_list.sort()
    for i in range(0, len(file_list)):
        _file = pd.read_csv(os.path.join("./../data/austria/old_data", file_list[i]))
        _file_2 = pd.read_csv(os.path.join("./../data/germany/old_data", file_list[i]))
        total = pd.DataFrame()
        total['week'] = _file['week']
        total['incidence'] = stz(stz(_file['incidence']) + stz(_file_2['incidence']))
